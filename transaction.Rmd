...
ggplot(train_test) +
 aes(x = date, y = mean_amount, colour = type) +
 geom_line(size = 0.5) +
 scale_color_hue(direction = 1) +
...
dec_2016 <- data.frame(date = as.Date("01/12/2016","%d/%m/%Y"),
                      industry="1",
                      location="1",
                      mean_amount=0,
                      month_number="12",
...
# train_set_1_1$mean_amount <- predict(M1_1,dec, interval="prediction")
# test_set_1_1$mean_amount <- predict(M1_1,test_set_1_1, interval="prediction")
# 
# # To add this to the existing dataset (agg_1_1) we use rbind
# agg_data_with_dec_pred_1 <- rbind(train_set_1_1, test_set_1_1)
# rmarkdown::paged_table(agg_data_with_dec_pred_1)

## Use predict function + model we just built and apply to dec_2016 dataframe
dec_2016$mean_amount<- predict(M1_1, dec_2016, interval="prediction")
...
# actual_1 <- test_set_1_1[,1]

# pred_ts_1 <- ts(pred_1, [,2], start = year, 
...
#adjust dateformat
agg_1_1$date <- as.Date(agg_1_1$date,"%d/%m/%Y")
agg_data_with_dec_pred_1$date <- as.Date(agg_data_with_dec_pred_1$date,"%d/%m/%Y")
test_set_1_1$date <- as.Date(test_set_1_1$date,"%d/%m/%Y")

# add new column to specify the value type
agg_1_1$type <- "Real"
agg_data_with_dec_pred_1$type <- "Predicted"
test_set_1_1$type <- "Fitted"

# create data frame of actual and predicted values
predicted_real_1 <- sqldf(
  "SELECT * FROM test_set_1_1
  UNION ALL
  SELECT * FROM agg_data_with_dec_pred_1
  UNION ALL
  SELECT * FROM agg_1_1
  ORDER BY type;
  "
...
ggplot(predicted_real_1) +
  ...
  labs(
    ...
    title = "MLR Fitted",
    subtitle = "December 2016 transation",
    color = "Value type"
  ) +
  theme_bw() +
  theme(
...
ggplot(data = data.table(Fitted_values = M1_1$fitted.values,
                         Residuals = M1_1$residuals),
