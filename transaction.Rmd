...
A useful variation on the seasonal plot uses polar coordinates as the following illustration.
...
aggregate_transactions <- function(df) {
  ...
  output = df %>%
    ...
    summarize(mean_amount = mean(monthly_amount, na.rm = TRUE))
...
Now that we have a new adjusted dataset, I will use it to create a line plot of the variable `monthly_amount` for `industry = 1` and `location = 1` with the purpose of gaining more insights from targeted areas.
...
aggregated_transactions %>%
 ...
 1L) %>%
 ggplot() +
  aes(x = date, y = mean_amount, colour = year_number) +
  geom_line(size = 0.7) +
  geom_point(size=1) +
...
aggregated_transactions %>%
 ...
 ggplot() +
 aes(x = month_number, fill = year_number, weight = mean_amount) +
 geom_bar() +
...
Since the aim of this project is to build a model that can predict `monthly_amount`for the future, we first split the data into train and the test set. The train set includes three years of data from 2013 to 2016 while test set includes one last year of data, 2016.
...
Additionally, as we have 2 requirements in this assignment which is:
...
I will generate an additional data set that filtered only Industry 1 and Location 1 records by the following code. The train and split test for the Advanced Model Fitting section can be kept the same as there are no further adjustments needed.
...
In this section, I will focus on developing a MLR filtered by Location 1 & Industry 1 only. 
...
################################################################################
M1_1 <-lm(mean_amount~date+month_number, data =train_set_1_1)
summary(M1_1)
```
The month number variable is introduced to accommodate for the seasonality of the sales amount. As summarized in the linear model with the formula `formula = mean_amount ~ date + month_number`, this model performs quite impressively with the Adjusted R-Square equivalent to 0.7457. In other words, this indicates that approximately 74,57% observations in the training set are explained by the model.
...
################################################################################
M1_2 <-lm(mean_amount~month_number, data =train_set_1_1)
summary(M1_2)
```
...
# M3: 1 VARIABLE - date
################################################################################
M1_3 <-lm(mean_amount~date, data =train_set_1_1)
summary(M1_3)
```
...
# dec_2016 = data.frame(date = "2016-01-12",
#                       industry=1,
#                       location=1,
#                       mean_amount=0,
#                       month_number=12,
...
#                              format = "%y/%d/%m")
# dec_2016$industry <- as.numeric(dec_2016$industry)
# dec_2016$location <- as.numeric(dec_2016$location)
# dec_2016$month_number <- as.numeric(dec_2016$month_number)
# M1_1$month_number <- as.numeric(M1_1$month_number)
# 
# # Use predict function + model we just built and apply to dec_2016 dataframe
# dec_2016$mean_amount <- predict(M1_1,dec_2016)
# 
...
For projects that build predictive models, this would be the time to evaluate model performance on the hold-out test sets. For projects requiring statistical analysis, such as ours, statistical significance of the conclusions should be assessed. Additionally, we evaluate whether the model/analysis we have produced is correct and we haven’t overlooked anything 
