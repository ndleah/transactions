...
x <- c("date", "industry", "location", "mean_amount","month_number","year_number","type")
...
fit_df$mean_amount <- predict(M1_1, train_set_1_1)
pred_df$mean_amount <- predict(M1_1, test_set_1_1)
pred <- predict(M1_1, dec2016)
dec2016$mean_amount <- pred
pred_df <- sqldf(
  "SELECT * FROM pred_df 
  UNION ALL
  SELECT * FROM dec2016"
  ...
  for (ind in industries) {
    for (loc in locations) {
      ...
      if (length(unique(temp$date)) >= testingNum) {
        ...
        # Use data for the year 2016 to test accuracy
        testingSet <- temp[temp$date >=	"2016-01-01",]   
        # Training set
        # Use data for the year 2016 for forecasting
        trainingSet <- temp[(temp$date < "2016-01-01"),]
       
        # Run the model
        training.model = lm(mean_amount~date+month_number, data=trainingSet)
        ...
        temp = dplyr::bind_rows(temp, december_2016)
...
# Get the list of unique industries, sorted in numerical order 
industries_df <- sort(unique(aggregated_transactions$industry))

# Get the list of unique locations, sorted in numerical order
locations_df <- sort(unique(aggregated_transactions$location))

industries_df <- as.data.frame(industries_df)
locations_df <- as.data.frame(locations_df)
locations_df <- as.data.frame(locations_df)

# Calculate the predictions for all industry and location combinations
all_pred_df <- calculate_predictions(aggregated_transactions, industries_df, locations_df)
View(all_pred_df)
...
```{r}
test_error <- evaluation_metrics_test_1_1$RMSE/mean(test_set_1_1$mean_amount) 
test_error
```
